...
func NewPrettier(config PrettierConfig) *Prettier {
	...
	outputQueue := rabbit.NewRabbitOutputQueue(channel, props.PrettierP4_Output, comms.Outputs)
...
func (prettier *Prettier) Run() {
	...
	backupCallbackByInput := map[string]func(int, []string){props.FilterF4_Name: prettier.builder.LoadBackup}
